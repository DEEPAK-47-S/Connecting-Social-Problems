export const sendOtpEmail = async (
  ...
  const senderEmail = process.env.BREVO_SENDER_EMAIL || 'your_verified_email@gmail.com';

  // If we don't have a real API key configured, stop here (local dev mode)
  if (!apiKey) {
    console.log(`ℹ️ [EMAIL] BREVO_API_KEY not configured. Use the OTP above in console.`);
    return;
  }
  ...
  try {
    const res = await fetch('https://api.brevo.com/v3/smtp/email', {
      ...
      body: JSON.stringify({
        sender: {
          name: "Connecting Social Problem",
          email: senderEmail
        },
        to: [{
          email: to,
    ...
    if (!res.ok) {
      console.error(`⚠️ [EMAIL] Brevo API error sending to ${to}:`, data);
      console.log(`💡 [EMAIL] The OTP code (${otp}) is still valid and displayed in console above.`);
      return;
    }

    console.log(`✅ [EMAIL] Successfully delivered via Brevo API to: ${to} (Message ID: ${data.messageId})`);
  } catch (err: any) {
    console.error(`⚠️ [EMAIL] Failed to send email via Brevo API to ${to}:`, err.message);
    console.log(`💡 [EMAIL] The OTP code (${otp}) is still valid and displayed in console above.`);
  }
};
